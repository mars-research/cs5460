...
system that allows you to use GCC, make, GRUB2 and QEMU (CADE machines, your laptop
that runs Linux or Linux VM, e.g., WSL on Windows, and even macOS with cross
compilation via nix, etc.). However, an Linux environment is recommended, because installing GRUB2 and cross-compilation toolchain is harder on macOS.
**You don't need to set up xv6 for this assignment,
...
any public release is prohibited. This assignment explains how to
create a minimal x86_64 operating system kernel using the Multiboot standard. In fact, it will just boot and print `"Hello, world!"` on the screen, and then print `"Hello from
...
One of the amazing and terrible things about the x86_64 architecture is that it's maintained backwards compatibility throughout the years. This has been a competitive
...
One other advantage of using GRUB: it handles the transition from real mode to protected mode for us, skipping the first step. We then finish the job ourselves by
setting up paging and switching to 64-bit long mode. If you're curious about the kinds of things you would have needed to know, put "A20 line" into your favorite search
...
Again, we'll use math to let the computer calculate the sum for us. We add up the magic number, the architecture code, and the header length, and then subtract it from a big number. dd then puts that value into this spot in our file.
...
```
The `-f elf32` says that we want to output a file as 32bit ELF. TODO
...
bits 32
```
GRUB will boot us into protected mode, aka 32-bit mode (similar to how xv6 bootloader starts in 16bit real mode GRUB will be loaded by the BIOS and will switch into protected 32bit mode for us). Our bootstrap starts in `bits 32`, and later in the homework we will switch into 64 bits mode before calling C.
...
```
Recall that on macOS you will want to use the linker from the cross-compiling toolchain, with the name `x86_64-elf-ld`.
...
-m elf_i386
```
ask the linker to generate the 32bit. TODO
```
-T linker.ld
```
...
qemu-system-x86_64
```
We're running the `x86_64` variant of QEMU. In this homework the kernel itself will transition to 64-bit long mode, so this matches the target architecture directly. While we have a 32-bit kernel now, the QEMU emulates x86 64bit architecture. And since 32-bit code is part of it everything works.
```
-cdrom hello.iso
```
...
CFLAGS = -fno-pic -fno-pie -ffreestanding -fno-builtin -fno-strict-aliasing -O1 -Wall -MD -ggdb -m64 -mno-red-zone -fno-omit-frame-pointer -Werror -nostdlib -fno-stack-protector
...
1. Enable paging and switch to long mode.
2. Setup stack and call into main.
...
The 64-bit long mode enforces 

### Setting up GDT
GDT (Global Descriptor Table) is a data structure used by x86 architecture to define the properties of memory segments. In long mode, the segmentation model is no longer used, but we still need GDT to mark the 64-bit code segment, to distinguish user and kernel privilege levels, and to set up the Task State Segment (TSS) for handling interrupts. For this assignment, we will set up a simple GDT with just a code segment and a data segment, both covering the entire address space.
...
In `long_mode_entry`, load data segments and continue in `bits 64`.

##### Your assignment is to implement the GDT + long-mode switch described above
### Setting up the stack
Now, since we have a GDT and paging setup, we can set up the stack and jump into main.
...
```
Then initialize the stack in 64-bit mode like:
```
mov rsp, stack + 4096
call main
```
...
char c_stack[PGSIZE];
```
And then this in assembly to load it into the `rsp` register
```
...
extern c_stack
  ...
  mov rsp, c_stack + 4096
```

### Creating the page table
Now we can create the full 4-level page table in C and map the first 8MB with identity mapping.

```
__attribute__((__aligned__(PGSIZE))) uint64_t entry_pml4[512];
__attribute__((__aligned__(PGSIZE))) uint64_t entry_pdpt[512];
__attribute__((__aligned__(PGSIZE))) uint64_t entry_pd[512];
__attribute__((__aligned__(PGSIZE))) uint64_t entry_pt[4][512];

// PML4[0] -> PDPT[0] -> PD[0..3] -> PT[0..3]
// This maps VA [0, 8MB) to PA [0, 8MB).
```

Populate `entry_pt[t][i]` with:

```
((t * 512 + i) * 0x1000) | PTE_P | PTE_W
```

Then link levels:

1. `entry_pml4[0] = (uint64_t)entry_pdpt | PTE_P | PTE_W`
2. `entry_pdpt[0] = (uint64_t)entry_pd   | PTE_P | PTE_W`
3. `entry_pd[0..3] = (uint64_t)entry_pt[0..3] | PTE_P | PTE_W`

Inside `main()`, load your final page table by updating `CR3`:

```
write_cr3((uint64_t)&entry_pml4);
```

Paging is already enabled before entering `main()`; `main()` should build and load the final 8MB mapping.
### Compiling main()
...
Finally, **your assignment is to implement all the boot code that we've discussed above, switch into 64-bit long mode, and construct a 4-level page table that maps the first 8MB of virtual addresses to the first 8MB of physical memory**. Build and load the final mapping once you boot into `main()`.
